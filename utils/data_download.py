...
class DownThread(threading.Thread):
    ...
    def download_wget_os(self, url:str, outpath:str):
        cmd = 'wget -c ' + url + ' -P ' + outpath + ' -t ' + str(self.trytime) + ' -T ' + str(self.timeout)
...
def redownload_error_pretarining_data(url_list_timeout:str, url_list_wrang:str, outpath:str):
    max_process_number = 11
    url_list = []
...
def download_shsmu_as(allosteric_site_path:str, outpath:str):
    base_url = 'https://mdl.shsmu.edu.cn/ASD2023Common/static_file//site/allo_site_pdb_gz/'
    filename_extension = '.pdb.gz'
    ...
    for item in tqdm(data):
        if item['allosteric_site'] not in filename:
            wget.download(base_url+item['allosteric_site']+filename_extension, outpath+item['allosteric_site']+filename_extension)
...
def download_rcsb(allosteric_site_path:str, outpath:str):
    ...
    for item in tqdm(data):
        if item['allosteric_pdb'] not in filename:
            wget.download(base_url+item['allosteric_pdb']+filename_extension, outpath+item['allosteric_pdb']+filename_extension)
...
redownload_error_pretarining_data('../data/pretrain/timeout.log', '../data/pretrain/wrang.log', '../data/pretrain/')